...
(function(e) {
	...
	function u(r, i) {
		var s = t ? i : g(i);
		var o = s.worker && Papa.WORKERS_SUPPORTED && n;
		if (o) {
			var u = d();
			u.userStep = s.step;
			u.userChunk = s.chunk;
			u.userComplete = s.complete;
			u.userError = s.error;
			s.step = b(s.step);
			s.chunk = b(s.chunk);
			s.complete = b(s.complete);
			s.error = b(s.error);
			delete s.worker;
			u.postMessage({
				input: r,
			...
			} else if (e.File && r instanceof File || r instanceof Object) {
				if (s.step || s.chunk) {
					var a = new l(s);
					a.stream(r)
				} else {
					var h = new c(s);
					if (t) {
						var v = new FileReaderSync;
						var m = v.readAsText(r, s.encoding);
						return h.parse(m)
					} else {
						v = new FileReader;
						v.onload = function(e) {
						...
						v.onerror = function() {
							if (b(s.error)) s.error(v.error, r)
						};
						v.readAsText(r, s.encoding)
					}
				}
			}
		}
	...
	function a(t, n) {
		...
		function h(e, t) {
			for (var n = 0; n < t.length; n++)
				if (e.indexOf(t[n]) > -1) return true;
			return false
		}
		var r = "";
		var i = [];
	...
	function f(n) {
		function m() {
			...
			a.open("GET", f, !t);
			if (n.step || n.chunk) {
				var e = r + v.chunkSize - 1;
				if (i && e > i) e = i;
				a.setRequestHeader("Range", "bytes=" + r + "-" + e)
			}
			try {
				a.send()
			} catch (s) {
		...
		function g() {
			...
			o += u + a.responseText;
			u = "";
			h = !n.step && !n.chunk || r > E(a);
			if (!h) {
				var i = o.lastIndexOf("\r");
				if (i == -1) i = o.lastIndexOf("\n");
				if (i != -1) {
					u = o.substring(i + 1);
					o = o.substring(0, i)
				} else {
			...
			var f = d.parse(o);
			o = "";
			if (f && f.data) s += f.data.length;
			var c = h || v.preview && s >= v.preview;
			if (t) {
				e.postMessage({
					results: f,
			...
			if (b(p) && c) p(f);
			if (!c && (!f || !f.meta.paused)) l()
		}

		function w(r) {
			var i = a.statusText || r;
			if (b(n.error)) n.error(i);
			else if (t && n.error) {
				e.postMessage({
					workerId: Papa.WORKER_ID,
					error: i,
		...
		function S(e) {
			v = y(e);
			p = v.complete;
			v.complete = undefined;
			v.chunkSize = parseInt(v.chunkSize);
			d = new c(v);
			d.streamer = this
		}
		n = n || {};
		if (!n.chunkSize) n.chunkSize = Papa.RemoteChunkSize;
		var r = 0,
			i = 0,
			s = 0;
		var o = "";
	...
	function l(n) {
		function p() {
			if (!d && (!g.preview || a < g.preview)) E()
		}

		function E() {
			var e = Math.min(r + g.chunkSize, i.size);
			var t = h.readAsText(s.call(i, r, e), n.encoding);
			if (!w) S({
				target: {
					result: t
				}
		...
		function S(s) {
			...
			d = r >= i.size;
			if (!d) {
				var c = o.lastIndexOf("\r");
				if (c == -1) c = o.lastIndexOf("\n");
				if (c != -1) {
					u = o.substring(c + 1);
					o = o.substring(0, c)
				} else {
			...
			var h = m.parse(o);
			o = "";
			if (h && h.data) a += h.data.length;
			var y = d || g.preview && a >= g.preview;
			if (t) {
				e.postMessage({
					results: h,
					workerId: Papa.WORKER_ID,
			...
			if (b(v) && y) v(h);
			if (!y && (!h || !h.meta.paused)) p()
		}

		function x() {
			if (b(n.error)) n.error(h.error, i);
			else if (t && n.error) {
				e.postMessage({
					workerId: Papa.WORKER_ID,
					error: h.error,
		...
		function T(e) {
			g = y(e);
			v = g.complete;
			g.complete = undefined;
			g.chunkSize = parseInt(g.chunkSize);
			m = new c(g);
			m.streamer = this
		}
		n = n || {};
		if (!n.chunkSize) n.chunkSize = Papa.LocalChunkSize;
		var r = 0;
		var i;
		var s;
		var o = "";
		...
		var v, m, g;
		T(n);
		var w = typeof FileReader !== "undefined";
		this.stream = function(e) {
			i = e;
			s = i.slice || i.webkitSlice || i.mozSlice;
			if (w) {
				h = new FileReader;
				h.onload = S;
				h.onerror = x
			} else h = new FileReaderSync;
			p()
		};
		this.finished = function() {
			return d
	...
	function c(e) {
		function c() {
			if (f && u) {
				E("Delimiter", "UndetectableDelimiter", "Unable to auto-detect delimiting character; defaulted to '" + Papa.DefaultDelimiter + "'");
				u = false
			}
			if (e.skipEmptyLines) {
				for (var t = 0; t < f.data.length; t++)
					if (f.data[t].length == 1 && f.data[t][0] == "") f.data.splice(t--, 1)
			}
			if (p()) d();
			return v()
		}
		...
		function d() {
			if (!f) return;
			for (var e = 0; p() && e < f.data.length; e++)
				for (var t = 0; t < f.data[e].length; t++) a.push(f.data[e][t]);
			f.data.splice(0, 1)
		}
		...
		function v() {
			...
			for (var t = 0; t < f.data.length; t++) {
				...
				if (e.header) {
					f.data[t] = n;
					if (r > a.length) E("FieldMismatch", "TooManyFields", "Too many fields: expected " + a.length + " fields but parsed " + r, t);
					else if (r < a.length) E("FieldMismatch", "TooFewFields", "Too few fields: expected " + a.length + " fields but parsed " + r, t)
				}
			}
		...
		this.parse = function(t) {
			...
			if (!e.delimiter) {
				var r = m(t);
				if (r.successful) e.delimiter = r.bestDelimiter;
				else {
					u = true;
			...
			var a = y(e);
			if (e.preview && e.header) a.preview++;
			i = t;
			s = new h(a);
			f = s.parse(i);
			c();
			if (b(e.complete) && !o && (!n.streamer || n.streamer.finished())) e.complete(f);
			return o ? {
				meta: {
					paused: true
				}
		...
		this.resume = function() {
			...
			s.parse(i);
			if (!o) {
				if (n.streamer && !n.streamer.finished()) n.streamer.resume();
				else if (b(e.complete)) e.complete(f)
			}
		};
		this.abort = function() {
			s.abort();
			if (b(e.complete)) e.complete(f);
			i = ""
		}
	}
		...
		this.parse = function(e) {
			...
			if (o) {
				...
				for (var y = 0; y < g.length; y++) {
					if (r && g[y].substr(0, h) == r) continue;
					if (p) {
						d = [g[y].split(t)];
						N();
						if (a) return T()
					} else d.push(g[y].split(t));
					if (s && y >= s) {
						d = d.slice(0, s);
						return T(true)
					}
			...
			for (;;) {
				if (e[u] == '"') {
					...
					for (;;) {
						...
						if (E == f - 1) {
							m.push(e.substring(u, E).replace(/""/g, '"'));
							d.push(m);
							if (p) N();
							return T()
						}
						...
						if (e.substr(E + 1, c) == n) {
							...
							if (s && d.length >= s) return T(true);
							break
						}
					}
					continue
				}
				if (r && m.length == 0 && e.substr(u, h) == r) {
					if (w == -1) return T();
					u = w + c;
					w = e.indexOf(n, u);
					b = e.indexOf(t, u);
				...
				if (b != -1 && (b < w || w == -1)) {
					m.push(e.substring(u, b));
					u = b + l;
					b = e.indexOf(t, u);
					continue
				}
				if (w != -1) {
					m.push(e.substring(u, w));
					x(w + c);
					if (p) {
						N();
						if (a) return T()
	...
	function v(e) {
		...
		if (t.finished) {
			if (b(r[t.workerId].userComplete)) r[t.workerId].userComplete(t.results);
	...
	function g(e) {
		...
		var t = y(e);
		if (typeof t.delimiter !== "string" || t.delimiter.length != 1 || Papa.BAD_DELIMITERS.indexOf(t.delimiter) > -1) t.delimiter = s.delimiter;
		if (t.newline != "\n" && t.newline != "\r" && t.newline != "\r\n") t.newline = s.newline;
		if (typeof t.header !== "boolean") t.header = s.header;
		if (typeof t.dynamicTyping !== "boolean") t.dynamicTyping = s.dynamicTyping;
		if (typeof t.preview !== "number") t.preview = s.preview;
		if (typeof t.step !== "function") t.step = s.step;
		if (typeof t.complete !== "function") t.complete = s.complete;
		if (typeof t.error !== "function") t.error = s.error;
		if (typeof t.encoding !== "string") t.encoding = s.encoding;
		if (typeof t.worker !== "boolean") t.worker = s.worker;
		if (typeof t.download !== "boolean") t.download = s.download;
		if (typeof t.skipEmptyLines !== "boolean") t.skipEmptyLines = s.skipEmptyLines;
		if (typeof t.fastMode !== "boolean") t.fastMode = s.fastMode;
		return t
	}
	...
	if (t) e.onmessage = m;
	else if (Papa.WORKERS_SUPPORTED) n = p()
})(this);